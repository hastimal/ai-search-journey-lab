...
def render_app() -> None:
    ...
    with tab_fanout:
        ...
        for q in journey.fanout:
            badge_class = "badge-places" if q.tool == ToolName.GOOGLE_PLACES else "badge-search"
            badge_label = "GOOGLE PLACES" if q.tool == ToolName.GOOGLE_PLACES else "GOOGLE SEARCH"
    ...
    # ---------------- TAB 3: Constraint Matrix & Metrics ----------------
    with tab_matrix:
        st.subheader("📊 Retrieval Summary Metrics")
        m1, m2, m3, m4, m5 = st.columns(5)
        places_tasks_count = len([q for q in journey.fanout if q.tool == ToolName.GOOGLE_PLACES])
        search_tasks_count = len([q for q in journey.fanout if q.tool == ToolName.GOOGLE_SEARCH])
