        ...
        *,
        ...
        with trace_span(
            ...
            attributes={
                ...
                "journey.query_length": len(question),
            },
            run_id=run_id,
        ):
            ...
            try:
                with trace_span("journey.intent_parsing", run_id=run_id) as s_span:
                    intent = await extract_intent(question)
                    if s_span.is_recording():
                        s_span.set_attribute("intent.category", str(intent.category or "unknown"))
                        s_span.set_attribute(
                            "intent.has_location", bool(intent.reference_location)
                        )
                        s_span.set_attribute(
                            "intent.constraint_count", len(intent.hard_constraints)
                        )
            ...
            try:
                with trace_span("journey.reference_location", run_id=run_id) as s_span:
                    if intent.reference_location:
                        ...
                        if ref_loc:
                            ...
                            if s_span.is_recording():
                                s_span.set_attribute("location.resolved", True)
                        else:
                            step_ref.detail = f"Not resolved: '{intent.reference_location}'"
                            trace(
                                f"Could not resolve reference location for: "
                                f"'{intent.reference_location}'"
                            )
                            if s_span.is_recording():
                                s_span.set_attribute("location.resolved", False)
                    else:
                        step_ref.detail = "No reference location requested"
                        trace("No reference location specified in intent")
                        if s_span.is_recording():
                            s_span.set_attribute("location.requested", False)
            ...
            t0 = time.perf_counter()
            try:
                with trace_span("journey.query_fanout", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording():
                        s_span.set_attribute("fanout.total_queries", len(fanout))
                        s_span.set_attribute("fanout.places_queries", p_cnt)
                        s_span.set_attribute("fanout.search_queries", s_cnt)

                step_fanout.duration_seconds = round(max(0.0, time.perf_counter() - t0), 3)
                step_fanout.status = StepExecutionStatus.COMPLETED
                step_fanout.detail = (
                    f"{p_cnt} Places · {s_cnt} Search tasks ({len(fanout)} total)"
                )
                notify_step(step_fanout)
            ...
            try:
                with trace_span("journey.places_retrieval", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording():
                        s_span.set_attribute("places.task_count", len(places_tasks))
                        s_span.set_attribute("places.raw_count", total_raw)
            ...
            t0 = time.perf_counter()
            try:
                with trace_span("journey.normalize", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording():
                        s_span.set_attribute("normalize.raw_count", total_raw)
                        s_span.set_attribute("normalize.unique_count", len(canonical_candidates))
                        s_span.set_attribute("normalize.duplicates_removed", dupes_removed)
            ...
            try:
                with trace_span("journey.search_grounding", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording():
                        s_span.set_attribute("search.task_count", len(search_tasks))
                        s_span.set_attribute("search.executed_queries_count", total_queries)
                        s_span.set_attribute("search.sources_count", total_sources)

                step_search.duration_seconds = round(max(0.0, time.perf_counter() - t0), 3)
                step_search.status = StepExecutionStatus.COMPLETED
                step_search.detail = (
                    f"{len(search_tasks)} tasks · {total_queries} queries · "
            ...
            try:
                ...
                with trace_span("journey.evidence_aggregation", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording():
                        s_span.set_attribute("evidence.claims_count", len(all_evidence))
                        s_span.set_attribute(
                            "evidence.candidates_count", len(evidence_result.candidates)
                        )
            ...
            t0 = time.perf_counter()
            try:
                with trace_span("journey.constraint_evaluation", run_id=run_id) as s_span:
                    constraint_matrix = evaluate_constraints(intent, evidence_result.candidates)
                    if s_span.is_recording():
                        s_span.set_attribute(
                            "constraints.evaluations_count", len(constraint_matrix.evaluations)
                        )
            ...
            t0 = time.perf_counter()
            try:
                with trace_span("journey.ranking", run_id=run_id) as s_span:
                    ranked_all = rank_candidates(constraint_matrix, reference_location=ref_loc)
                    top_candidates = ranked_all[:3]
                    if s_span.is_recording():
                        s_span.set_attribute("ranking.candidates_count", len(ranked_all))
                        s_span.set_attribute("ranking.top_count", len(top_candidates))
            ...
            try:
                with trace_span("journey.static_map", run_id=run_id) as s_span:
                    static_map_result = generate_static_map(top_candidates, max_candidates=3)
                    if s_span.is_recording():
                        s_span.set_attribute(
                            "static_map.markers_count", static_map_result.marker_count
                        )
                ...
                step_map_preview.detail = (
                    f"{static_map_result.marker_count} map markers generated"
                )
                notify_step(step_map_preview)
            ...
            try:
                with trace_span("journey.answer_synthesis", run_id=run_id) as s_span:
                    ...
                    if s_span.is_recording() and grounded_answer:
                        s_span.set_attribute(
                            "answer.recommendations_count", len(grounded_answer.recommendations)
                        )
                        s_span.set_attribute(
                            "answer.caveats_count", len(grounded_answer.caveats)
                        )

                step_ans.duration_seconds = round(max(0.0, time.perf_counter() - t0), 3)
                step_ans.status = StepExecutionStatus.COMPLETED
                step_ans.detail = (
                    f"{len(grounded_answer.recommendations)} candidate recommendations"
                )
                notify_step(step_ans)
